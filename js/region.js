...
(async () => {
    ...
    const changeRegionForm = document.querySelector('form[action="page=change_region"]');
    ...
    const currentRegionName = urlParameters['region'];
    const detaggingDiv = document.createElement('div');
    ...
    async function moveToRegion(e) {
        e.preventDefault();
        const localId = document.querySelector('input[name=localid]').getAttribute('value');
    ...
    let nationsEndorsed = [];
    async function actionButtonClick(e) {
        ...
        if (value === 'Refresh') {
            ...
            let updateTime = responseElement.querySelector('#regioncontent > p:nth-child(4) > time').innerHTML;
            let waDelegate;
            try {
            ...
            chrome.storage.local.get(['switchers', 'prepswitchers'], async (result) => {
                let switchers = orderSwitchersByPrepList(result.switchers ?? [], result.prepswitchers ?? []);
                if (!switchers.length) {
    ...
    async function refreshEndorseList(e) {
        ...
        for (let i = 0; i != happeningsText.length; i++) {
            ...
            // don't let us endorse the same nation twice
                ...
                if (resigned.indexOf(nationName) === -1) {
                    ...
                    let endorseButton = document.createElement('input');
    ...
    async function manualLocalIdUpdate(e) {
        document.querySelector('#update-localid').setAttribute('data-clicked', '1');
        const response = await makeAjaxQuery('/page=upload_flag', 'GET');
        const responseElement = document.createRange().createContextualFragment(response);
        const localIdElement = responseElement.querySelector('input[name=localid]');
        const localId = localIdElement?.getAttribute('value');
        if (!localId) {
            document.querySelector('#endorse-status').innerHTML = 'Failed to update localid.';
            return;
        }
        chrome.storage.local.set({ 'localid': localId });
        document.querySelector('#endorse-status').innerHTML = 'Updated localid.';
    }
    function updateChk(page) {
